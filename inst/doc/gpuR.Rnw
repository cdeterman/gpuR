...
\Rclass{gpuMatrix} and \Rclass{gpuVector} objects.  These methods include
\Rcode{\%*\%}, \Rcode{'+'}, \Rcode{'-'}, \Rcode{crossprod}, \Rcode{tcrossprod}, 
\Rcode{colMeans}, \Rcode{colSums}, \Rcode{rowMean}, \Rcode{rowSums}, 
\Rcode{cov} (Pearson Covariance), and \Rcode{eigen}. With many more in 
development.
