...
template <class T> 
#ifdef BACKEND_CUDA
class dynVCLMat<T, typename std::enable_if<std::is_floating_point<T>::value>::type> {
#else
class dynVCLMat<T>{
#endif
    ...
    public:
        ...
        dynVCLMat(SEXP A_, int ctx_id): ctx_id(ctx_id) {
            ...
            viennacl::matrix<T> A = viennacl::matrix<T>(K,M);
#else
            // explicitly pull context for thread safe forking
            viennacl::context ctx = viennacl::context(viennacl::ocl::get_context(static_cast<long>(ctx_id)));
        ...
        dynVCLMat(
            ...
            viennacl::matrix<T> A = viennacl::matrix<T>(nr_in, nc_in);
#else
            // explicitly pull context for thread safe forking
            viennacl::context ctx = viennacl::context(viennacl::ocl::get_context(static_cast<long>(ctx_id)));
        ...
        dynVCLMat(int nr_in, int nc_in, int ctx_id): ctx_id(ctx_id) {   
            ...
            viennacl::matrix<T> A = viennacl::zero_matrix<T>(nr_in, nc_in);
#else
            // explicitly pull context for thread safe forking
            viennacl::context ctx = viennacl::context(viennacl::ocl::get_context(static_cast<long>(ctx_id)));
        ...
        dynVCLMat(int nr_in, int nc_in, T scalar, int ctx_id): ctx_id(ctx_id) {
            ...
            viennacl::matrix<T> A = viennacl::scalar_matrix<T>(nr_in, nc_in, scalar);
#else
            // explicitly pull context for thread safe forking
            viennacl::context ctx = viennacl::context(viennacl::ocl::get_context(static_cast<long>(ctx_id)));
